		...
		createElements : function($target) {
			...
			if (!$dropdown) {
				...
				'<div id="datepicker-dropdown">'+
					...
					'<div class="date-ranges-form">'+
           			...
           			'</div>'+
						...
						'<button class="btn btn-primary btn-sm" id="button-ok">Apply</button>'+
						'<button class="btn btn-sm" id="button-cancel">Cancel</button>'+
						'</div>'+
					'</div>'+
				'</div>');
				...
				$('#button-ok', $dropdown).click(function() {
					...
					window.dr1_from = $('.dr1.from', $dropdown).val();
					console.log("changed the value!, now dr1_from:"+dr1_from);
					console.log($('.dr1.from', $dropdown).val());
					console.log($('.dr1.to', $dropdown).val());
					console.log($('.dr2.from', $dropdown).val());
					console.log($('.dr2.to', $dropdown).val());*/
					getDatesFromPicker();
